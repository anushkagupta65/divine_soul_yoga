...
class MyMapPage {
  final String destination;// Declare the order variable

  MyMapPage(this.destination);

  Future<void> _getCurrentLocationAndOpenMap() async {
    try {
      Position position = await _getCurrentLocation();
      String destinationAddress = destination; // The destination address from your order
...
void startNavigation(String destination) {
  MyMapPage(destination).navigateToDestination();
}
